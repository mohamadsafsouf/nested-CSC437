...
final class SessionPersistenceService {
    ...
    private let logger = Logger(subsystem: "CamGuardiOS", category: "SessionPersistence")
    private let userKey = "camguard.ios.session.user"
    private let lastSelectedTabKey = "camguard.ios.session.lastSelectedTab"
    private let keychainService = "com.camguard.ios.session"
    ...
    func saveUser(_ user: AppUser) {
        do {
            userDefaults.set(try JSONEncoder().encode(user), forKey: userKey)
        } catch {
